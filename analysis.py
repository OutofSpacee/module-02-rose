...
# Get the set of streamers we're keeping
...
# Add nodes with viewer_count as a node attribute
...
# Get viewer counts for node sizing
viewer_counts = [g.nodes[node]['viewer_count'] for node in g.nodes()]

# Normalize viewer counts for node size (scale for visibility)
min_size = 20
max_size = 800
...
# Add labels for top 30 streamers only
...
if g.number_of_nodes() > 0:
    ...
    # --- Top 3 most important nodes by centrality ---
    ...
    # Eigenvector centrality (may fail if not connected)
    try:
